...
use crate::content::{self, flash, jxl, pdf, ugoira};
...
pub fn representative_image(config: &Config, file_path: &Path, mime_type: MimeType) -> ApiResult<DynamicImage> {
    match mime_type {
        ...
        MimeType::Jxl => jxl::image(config, file_path),
        MimeType::Pdf => pdf::preview_image(file_path),
...
pub fn detect_post_type(config: &Config, file_path: &Path, mime_type: MimeType) -> ApiResult<PostType> {
    ...
    match mime_type {
        ...
        MimeType::Jxl => jxl::is_animated(config, file_path).map(image_type),
