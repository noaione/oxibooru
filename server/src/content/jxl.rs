...
pub fn image(config: &Config, file_path: &Path) -> ApiResult<DynamicImage> {
    ...
    let mut decoder = jxl_read_info(config, &mut input)?;
    let info = decoder.basic_info();

    let (width, height) = info.size;
    if u32::try_from(width)
        .ok()
        .is_none_or(|width| width > config.limits.max_image_width)
        || u32::try_from(height)
            .ok()
            .is_none_or(|height| height > config.limits.max_image_height)
    {
        return Err(ApiError::from(LimitErrorKind::DimensionError));
    }
    ...
    let max_allocation = usize::try_from(config.limits.max_image_allocation).unwrap_or(usize::MAX);
    let buffer_len = width
        .checked_mul(height)
        .and_then(|pixel_count| pixel_count.checked_mul(samples_per_pixel))
        .filter(|&len| len <= max_allocation)
        .ok_or(ApiError::from(LimitErrorKind::InsufficientMemory))?;
    let bytes_per_row = samples_per_pixel * width; // Cannot overflow after the check above

    let mut pixel_data = vec![0; buffer_len];
...
pub fn is_animated(config: &Config, path: &Path) -> ApiResult<bool> {
    let file = content::map_read_result(File::open(path))?;
    let mut input = BufReader::new(file);
    jxl_read_info(config, &mut input).map(|decoder| decoder.basic_info().animation.is_some())
}
...
fn jxl_read_info<In: JxlBitstreamInput>(config: &Config, input: &mut In) -> ApiResult<JxlDecoder<WithImageInfo>> {
